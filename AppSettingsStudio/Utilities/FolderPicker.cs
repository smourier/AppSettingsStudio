...
// like FolderBrowserDialog but allows to select non file-system items (such as \\wsl... etc.)
...
#pragma warning restore CA1826 // Do not use Enumerable methods on indexable collections
    ...
    public bool? ShowDialog(IWin32Window window, bool throwOnError = false) => ShowDialog(window?.Handle ?? nint.Zero, throwOnError);
    public virtual bool? ShowDialog(nint owner, bool throwOnError = false)
    {
        var hr = CoCreateInstance(CLSID_FileOpenDialog, 0, CLSCTX_ALL, typeof(IFileOpenDialog).GUID, out var dialog);
        if (hr < 0)
            return null;

        if (!string.IsNullOrEmpty(InputPath))
        {
            if (CheckHr(SHCreateItemFromParsingName(InputPath, 0, typeof(IShellItem).GUID, out var item), throwOnError) != 0)
                return null;
    ...
    private static partial int SHCreateItemFromParsingName([MarshalAs(UnmanagedType.LPWStr)] string? pszPath, nint pbc, in Guid riid, out IShellItem ppv);

    [LibraryImport("user32")]
    private static partial nint GetDesktopWindow();

    // hack: this definition is hardcoded for IFileOpenDialog
    [LibraryImport("ole32")]
    private static partial int CoCreateInstance(in Guid rclsid, nint pUnkOuter, int dwClsContext, in Guid riid, out IFileOpenDialog ppv);
